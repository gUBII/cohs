<?php
    include("auth.php");
    if($uactype=="ADMIN"){
        echo"<div class='content-header'><div><h4 class='content-title card-title'>Email Server Setting </h2></div><div> </div></div>
        <form method='post' action='dataprocessor.php' target='dataprocessor' enctype='multipart/form-data' id='loginForm' class='tooltip-end-bottom' novalidate >";
            ...
            if ($r1->num_rows > 0) { while($rs1 = $r1->fetch_assoc()) {
                echo"<input type=hidden name='processor' value='companysetting8'><input type=hidden name='id' value='".$rs1["id"]."'>
                <div class='row border-bottom mb-4 pb-4'>
                    <div class='col-md-12' style='margin-top:15px'><label class='form-label'>Email Address</label><input class='form-control' type='text' name='send_email' placeholder='Type here' value='".$rs1["send_email"]."'></div>
                    <div class='col-md-8' style='margin-top:15px'><label class='form-label'>SMTP Address</label><input class='form-control' type='text' name='smtp_server' placeholder='Type here' value='".$rs1["smtp_server"]."'></div>
                    <div class='col-md-4' style='margin-top:15px'><label class='form-label'>SMTP Port</label><input class='form-control' type='text' name='smtp_port' placeholder='Type here' value='".$rs1["smtp_port"]."'></div>
