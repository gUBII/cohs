<?php
    include("auth.php");
    if($uactype=="ADMIN"){
        echo"<div class='content-header'><div><h4 class='content-title card-title'>Payment API Setting </h2></div><div> </div></div>
        <form method='post' action='dataprocessor.php' target='dataprocessor' enctype='multipart/form-data' id='loginForm' class='tooltip-end-bottom' novalidate >";
            ...
            if ($r1->num_rows > 0) { while($rs1 = $r1->fetch_assoc()) {
                echo"<input type=hidden name='processor' value='companysetting9'><input type=hidden name='id' value='".$rs1["id"]."'>
                <div class='row border-bottom mb-4 pb-4'>
                    <div class='col-md-6' style='margin-top:15px'><label class='form-label'>Project Path</label><input class='form-control' type='text' name='project_path' placeholder='Type here' value='".$rs1["project_path"]."'></div>
                    <div class='col-md-6' style='margin-top:15px'><label class='form-label'>API Domain URL</label><input class='form-control' type='text' name='api_domain_url' placeholder='Type here' value='".$rs1["api_domain_url"]."'></div>
                    <div class='col-md-6' style='margin-top:15px'><label class='form-label'>Store ID</label><input class='form-control' type='text' name='store_id' placeholder='Type here' value='".$rs1["store_id"]."'></div>
                    <div class='col-md-6' style='margin-top:15px'><label class='form-label'>Store Password</label><input class='form-control' type='text' name='store_password' placeholder='Type here' value='".$rs1["store_password"]."'></div>
