<?php
    include("auth.php");
    if($uactype=="ADMIN"){
        echo"<div class='content-header'><div><h4 class='content-title card-title'>Notice Board Setting </h2></div><div> </div></div>
        <form method='post' action='dataprocessor.php' target='dataprocessor' enctype='multipart/form-data' id='loginForm' class='tooltip-end-bottom' novalidate >";
            ...
            if ($r1->num_rows > 0) { while($rs1 = $r1->fetch_assoc()) {
                echo"<input type=hidden name='processor' value='companysetting7'><input type=hidden name='id' value='".$rs1["id"]."'>
                <div class='row border-bottom mb-4 pb-4'>
                    <div class='col-md-9' style='margin-top:15px'><label class='form-label'>Notice Board</label><input class='form-control' type='text' name='noticeboard' placeholder='Type here' value='".$rs1["noticeboard"]."'></div>
                    <div class='col-md-3' style='margin-top:15px'><label class='form-label'>Status </label><select class='form-select' name='noticeboardstatus' required=''><option value='".$rs1["noticeboardstatus"]."'>".$rs1["noticeboardstatus"]."</option><option value='ON'>ON</option><option value='OFF'>OFF</option></select></div>
