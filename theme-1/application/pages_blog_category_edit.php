<?php
    include("auth.php");
    echo"<div class='row'>
            <div class='col-md-8'><h4 class='content-title card-title'>Blog Category Editor</h2></div>
            <div class='col-md-4'><button class='btn btn-primary' onclick=\"shiftdataV1('pages_blog_category', 'datashiftX')\">Add New</button></div>
    </div><hr>
        ...
        if($_GET["mid"]!=0){
                ...
                <div class='row border-bottom mb-4 pb-4'>
                    ...
                    <div class='col-md-6' style='margin-top:15px'><label class='form-label'>Parent Menu</label>
                        <select class='form-select' name='des1' required=''>";
                            ...
                            if($tomtom==0) echo"<option value='TOP BLOG CATEGORY'>TOP BLOG CATEGORY</option>";
                            echo"<option value=''><hr></option>";
                            echo"<option value='TOP BLOG CATEGORY'>TOP BLOG CATEGORY</option>";
                    ...
                    <div class='col-md-12' style='margin-top:15px'>
                        <button class='btn btn-primary' type='submit' onblur=\"shiftdataV1('pages_blog_category', 'datashiftX')\">Update</button> &nbsp;  
                        ...
                        <a href='dataprocessor.php?processor=deletetask&delid=".$rs1x["id"]."&tbl=sms_link' target='dataprocessor'>
                            <button class='btn btn-danger rounded font-md' type='button' onblur=\"shiftdataV1('pages_blog_category', 'datashiftX')\">Delete</button>
                        </a>
                    </div>
                </div>";
            } }
            ...
            <div style='padding:0px'>
                <div class='row border-bottom mb-4 pb-4'>
                    ...
                    <div class='col-md-6' style='margin-top:15px'><label class='form-label'>Parent Menu</label>
                        <select class='form-select' name='des1' required=''>
                            <option value='TOP BLOG CATEGORY'>TOP BLOG CATEGORY</option>";
                            $s1z = "select * from sms_link where des1='TOP BLOG CATEGORY' order by sorder asc";
                            $r1z = $conn->query($s1z);
                            if ($r1z->num_rows > 0) { while($rs1z = $r1z->fetch_assoc()) {
                                echo"<option value='".$rs1z["id"]."'>".$rs1z["pam"]."</option>";
                    ...
                    <div class='col-md-12' style='margin-top:15px'><hr></div>
                    <div class='col-md-12' style='margin-top:15px'>
                        <button class='btn btn-primary' type='submit' onblur=\"shiftdataV1('pages_blog_category', 'datashiftX')\">Save</button> &nbsp;  
                        <button class='btn btn-light rounded font-md' type='reset'>Reset</button>
                    </div>
                </div>
            </div>";
