<?php
    ...
    echo"<section class='content-main'>
        <div class='row'>
            <div class='col-md-10 content-header'><h2 class='content-title'>Edit NOTICE</h2></div>
            <div class='col-md-2'><button class='btn btn-primary' onclick=\"shiftdataV1('pages_notice', 'datashiftX')\">Add New</button></div>
        </div>
        <div class='card'>
            <div class='card-body'>
                <div class='row gx-5'>";
                if($uactype=="ADMIN"){
                    $s1 = "select * from sms_notice where id='".$_GET["fid"]."' order by id asc limit 1";
                	$r1 = $conn->query($s1);
                    if ($r1->num_rows > 0) { while($rs1 = $r1->fetch_assoc()) {
                        echo"<form method='post' action='dataprocessor.php' target='dataprocessor' enctype='multipart/form-data' id='loginForm' class='tooltip-end-bottom' novalidate >
                            <div class='row'>
                                <div class='col-md-6'>
                                    <div class='mb-3'><label for='product_name' class='form-label'>Notice Title</label>
                                ...
                                <div class='col-md-9'>
                                    <div class='mb-3'><label class='form-label'>Notice Detail</label><textarea placeholder='Type here' class='form-control' name='solution'>".$rs1["solution"]."</textarea></div>
                                </div>
                                <div class='col-md-3' style='margin-top:25px'>
                                    <div class='d-grid'>
                                        <button class='btn btn-primary' onblur=\"shiftdataV1('pages_notice', 'datashiftX')\">Update</button>
                                        <input type=hidden name=processor value='updatenotice'><input type=hidden name=id value='".$rs1["id"]."'>
                                    </div>
                                </div>
                            </div>
                        </form>";
