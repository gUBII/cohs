<?php
    ...
    if($_POST["processor"]=="createtemplate"){
        ...
        if($invoice_no2x==0){
            ...
            $sql = "insert into ndis_invoices (ledger_id,received_from,employeeid,invoice_no2,receipt_date,date_from,date_to,invoice_no) 
            VALUES ('".$_POST["ledger_id"]."','".$_POST["cname"]."','".$_POST["employeeid"]."','".$_POST["invoice_no2"]."','$invoice_date','$invoice_date','$invoice_date','".$_POST["invoice_no"]."')";
            if ($conn->query($sql) === TRUE){
                $sysdate=time();
    ...
    if($_POST["processor"]=="editinvoice"){
        ...
        $n1 = "select * from ndis_support_catelogue where support_item_number='".$_POST["ndis_item"]."' order by id asc limit 1";
        $nn1 = $conn->query($n1);
        ...
        VALUES ('$idate','$idate','$n_description','".$_POST["ledger_id"]."','2','2','".$_POST["participant"]."','$ip','$mycountry','1',
        '".$_POST["employeeid"]."','".$_POST["invoice_no2"]."','$date_from','$date_to','".$_POST["ndis_item"]."','".$_POST["frequency"]."','$n_rate',
        '0','$total_amount','".$_POST["selection"]."','$invoice_id2','1','".$_POST["invoiceid"]."')";
        if ($conn->query($sql) === TRUE) $tomtom=0;
    ...
    if($_POST["processor"]=="invoiceupdate"){
        $date_from=strtotime($_POST["date_from"]);
        $date_to=strtotime($_POST["date_to"]);
        $sql="update ndis_invoices_addon set date_from='$date_from',date_to='$date_to',narration='".$_POST["note"]."',ndis_item='".$_POST["ndis_item"]."',hours='".$_POST["thour"]."',rate='".$_POST["trate"]."',cr_amt='".$_POST["ttotal"]."' where id='".$_POST["id"]."'";
        if ($conn->query($sql) === TRUE) $tomtom=0;
