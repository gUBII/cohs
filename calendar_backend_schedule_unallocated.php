...
if (isset($_GET['action']) && $_GET['action'] === 'repeat_source_list') {
    ...
    if ($pid > 0) {
        ...
        $q = $conn->query("SELECT * FROM shifting_allocation_table WHERE $where AND accepted='0' ORDER BY id DESC");
        if ($q) {
            while ($r = $q->fetch_assoc()) {
                // resolve labels
                $clientLabel = '';
...
if (isset($_GET['action']) && $_GET['action'] === 'template_list') {
    ...
    $rows = [];
    if ($pid > 0) {
        $q = $conn->query("SELECT * FROM shifting_allocation_table WHERE projectid='$pid' AND status='1' AND accepted='0' ORDER BY id DESC");
        if ($q) {
            while ($r = $q->fetch_assoc()) {
                $empLabel = '';
...
if (isset($_GET['action']) && $_GET['action'] === 'template_get') {
    ...
    $q = $conn->query("SELECT * FROM shifting_allocation_table WHERE id='$id' AND accepted='0' LIMIT 1");
    if ($q && $q->num_rows > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'repeat_generate') {
        ...
        foreach ($ids as $tid) {
            ...
            $tq = $conn->query("SELECT * FROM shifting_allocation_table WHERE id='$tid' AND accepted='0' LIMIT 1");
            if (!$tq || $tq->num_rows === 0) {
    ...
    if ($action === 'template_save' || $action === 'template_update') {
        ...
        if ($action === 'template_save') {
            $idx = 1;
            $rMax = $conn->query("SELECT id FROM shifting_allocation_table WHERE accepted='0' ORDER BY id DESC LIMIT 1");
    ...
    if ($action === 'accept') {
        ...
        $conn->query("UPDATE shifting_allocation SET accepted='0' WHERE id='$id' LIMIT 1");
        echo json_encode(['status'=>'ok']);
        exit;
    }
