...
if (isset($_GET['action']) && $_GET['action'] === 'repeat_source_list') {
    ...
    if ($pid > 0) {
        ...
        $q = $conn->query("SELECT * FROM shifting_allocation_table WHERE $where ORDER BY id DESC");
        if ($q) {
            while ($r = $q->fetch_assoc()) {
                // resolve labels
                $clientLabel = '';
...
if (isset($_GET['action']) && $_GET['action'] === 'template_list') {
    ...
    $rows = [];
    if ($pid > 0) {
        $q = $conn->query("SELECT * FROM shifting_allocation_table WHERE projectid='$pid' AND status='1' ORDER BY id DESC");
        if ($q) {
            while ($r = $q->fetch_assoc()) {
                $empLabel = '';
...
if (isset($_GET['action']) && $_GET['action'] === 'template_get') {
    ...
    // Optional filter: accepted=1 (accepted) or accepted=0 (pending)
    if (isset($_GET['accepted']) && ($_GET['accepted'] === '0' || $_GET['accepted'] === '1')) {
        $where[] = "accepted='".(int)$_GET['accepted']."'";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'template_save' || $action === 'template_update') {
        ...
        if ($action === 'template_save') {
            $idx = 1;
            $rMax = $conn->query("SELECT id FROM shifting_allocation_table ORDER BY id DESC LIMIT 1");
    ...
    if ($action === 'activity_log') {
        // Optional: filter by shift/allocation id by matching it inside `note`.
        // NOTE: The provided schema for `autid` does not include a dedicated foreign key column.
        $limit = isset($data['limit']) ? max(1, min(500, intval($data['limit']))) : 200;
        $shiftId = isset($data['id']) ? intval($data['id']) : 0;

        $where = "";
        if ($shiftId > 0) {
            // Best-effort filter: match the id anywhere in the note text
            $needle = $conn->real_escape_string((string)$shiftId);
            $where = "WHERE note LIKE '%".$needle."%'";
        }

        $sql = "SELECT id, tran_type, date, ip, note, user_agent FROM autid $where ORDER BY id DESC LIMIT $limit";
        $res = $conn->query($sql);
        if (!$res) {
            http_response_code(500);
