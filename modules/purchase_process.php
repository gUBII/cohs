<?php
    ...
    echo"<div class='offcanvas-header'>
        ...
        <button type='button' class='btn-close text-reset' data-bs-dismiss='offcanvas' aria-label='Close' onclick=\"shiftdataV2('purchase_data.php?cid=$cid&sheba=$sheba', 'datatableX')\"></button>
    </div>
        ...
        <div>";
            if(isset($_GET["pno"]) and isset($_GET["pid"]) and isset($_GET["pfrom"]) and $_GET["pno"]!=0){
                echo"<div>";
                    ...
                    $rc1 = "select * from purchase_voucher where proj_id='".$_GET["pid"]."' and purchased_from='".$_GET["pfrom"]."' and purchase_no='".$_GET["pno"]."' order by purchase_no asc limit 1";
                    $rc2 = $conn->query($rc1);
                    if ($rc2->num_rows > 0) { while($rc3 = $rc2->fetch_assoc()) {
                        $purchaseno=$rc3["purchase_no"];
                        $pdate=date("Y-m-d",$rc3["purchase_date"]);
                        ...
                        $vendorid="";
                        $s1c = "select * from uerp_user where id='".$rc3["purchased_from"]."' order by id desc limit 1";
                        $r1c = $conn->query($s1c);
                        if ($r1c->num_rows > 0) { while($rs1c = $r1c->fetch_assoc()) {
                    ...
                    echo"<form method='post' action='retailprocessor.php' target='dataprocessor' enctype='multipart/form-data' id='loginForm' class='tooltip-end-bottom'>
                        <input type=hidden name='processor' value='edit_purchase_voucher'><input type=hidden name='user_id' value='$userid'>
                        <div class='row'>
                            <div class='col-2' style='margin-top:15px'><label class='form-label'>Purchase No.</label><input class='form-control' type='text' name='purchase_no' required='' placeholder='Type here' readonly value='$purchaseno'></div>
                            <div class='col-2' style='margin-top:15px'><label class='form-label'>Date</label><input class='form-control' type='date' name='purchase_date' required='' placeholder='Type here' value='$pdate'></div>
                            ...
                            <div class='col-2' style='margin-top:15px'><label class='form-label'>A/c Ledger</label>
                                <select class='form-select' name='ledger_id' required='' >";
                                    $s1bd = "select * from ledger_group where id='$purchase_v' and status='ON' order by id asc limit 1";
                                    $r1bd = $conn->query($s1bd);
                                    if ($r1bd->num_rows > 0) { while($rs1bd = $r1bd->fetch_assoc()) {
                            ...
                            <div class='col-2' style='margin-top:15px'><label class='form-label'>Vendor/Supplier Name</label>
                                <select class='form-select' name='purchased_from' required=''>$vendorid";
                            ...
                            <div class='col-1' style='margin-top:15px'><label class='form-label'>Add to List</label><button style='width:100%' class='btn btn-primary' type='submit' onblur=\"shiftdataV2('purchase_multi_edit.php?pid=$purchaseno&sheba=purchase_voucher', 'datatableXX')\">ADD</button></div>
                        </div>
                    </form>
                        ...
                        <div class='data-table-responsive-wrapper'><hr>
                            <table class='data-table data-table-pagination data-table-standard responsive nowrap stripe hover' id='myTable' style='width:100%'>
                                ...
                                <tbody id='datatableXX'>";
                                    include 'purchase_multi_edit.php';
                                echo"</tbody>
                            </table>
                        </div>
                    </div>
                ...
                echo"<div>";
                    ...
                    $purchaseno=1;
                    $invoiceno=1;
                    $rc1 = "select * from purchase_voucher where user_id='$userid' order by purchase_no desc limit 1";
                    $rc2 = $conn->query($rc1);
                    if ($rc2->num_rows > 0) { while($rc3 = $rc2->fetch_assoc()) {
                        if($rc3["order_status"]==0){
                            ...
                            if($rc3["purchase_no"]>=2) $purchaseno=($rc3["purchase_no"]+1);
                    ...
                    echo"<form method='post' action='retailprocessor.php' target='dataprocessor' enctype='multipart/form-data' id='loginForm' class='tooltip-end-bottom'>
                        <input type=hidden name='processor' value='new_purchase_voucher'><input type=hidden name='user_id' value='$userid'>
                        <div class='row'>
                            <div class='col-2' style='margin-top:15px'><label class='form-label'>Purchase No.</label><input class='form-control' type='text' name='purchase_no' required='' placeholder='Type here' readonly value='$purchaseno'></div>
                            <div class='col-2' style='margin-top:15px'><label class='form-label'>Date</label><input class='form-control' type='date' name='purchase_date' required='' placeholder='Type here' value='$pdate'></div>
                            <div class='col-2' style='margin-top:15px'><label class='form-label'>Invoice No.</label><input class='form-control' type='text' name='invoice_no' placeholder='Type here' value='$invoiceno' required=''></div>
                            ...
                            <div class='col-1' style='margin-top:15px'><label class='form-label'>Add to List</label><button style='width:100%' class='btn btn-primary' type='submit' onblur=\"shiftdataV2('purchase_multi_data.php?pid=$purchaseno&sheba=purchase_voucher', 'datatableXX')\">ADD</button></div>";
                    ...
                    <div class='data-table-rows slim col-md-12' id='sample'>
                        <div class='data-table-responsive-wrapper'><hr>
                            <table class='data-table data-table-pagination data-table-standard responsive nowrap stripe hover' id='myTable' style='width:100%'>
                                ...
                                <tbody id='datatableXX'>";
                                    include 'purchase_multi_data.php';
                                echo"</tbody>
                            </table>
                        </div>
                    </div>";
                    
                    // purchase_no,purchase_date,invoice_no,ledger_id,purchased_from,status,product_name,quantity,amount,sku,brand,category,note,size,weight,color,material,capacity,flavour,style,bundle,modal,age,gender,genetic
