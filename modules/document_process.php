        ...
        <form method='post' action='documentsprocessor.php' target='dataprocessor' enctype='multipart/form-data'>";
            if(isset($_GET["did"])){
                ...
                if ($rr1->num_rows > 0) { while($rs21 = $rr1->fetch_assoc()) {
                    ...
                    <input type='hidden' name='categoryid' value='".$rs21["categoryid"]."'>
                    <input type='hidden' name='employeeid' value='".$_GET["empid"]."'>
                    <div class='modal-body'> 
                        ...
                        <div class='col-12' style='margin-bottom:25px'><div class='form-group'>
                            ...
                            <select class='form-control m-b ' name='categoryid' required>";
                                ...
                                if($mtype=="ADMIN") $c77 = "select * from modules where parent='5275' OR parent='5276' order by id asc";
                    ...
                    <div class='modal-footer' class='col-md-12' style='margin-top:15px;padding:5px;border-width:0px'>";
                        ...
                        // echo"document_data2.php?status=$status2&categoryid=".$_GET["categoryid"]."&empid=$empid ";
                        
                        echo"<button type='button' class='btn btn-outline-primary' data-bs-dismiss='offcanvas' onclick=\"shiftdataV2('document_data2.php?status=1&categoryid=".$_GET["categoryid"]."&empid=".$_GET["empid"]."&url=".$_GET["url"]."&id=".$_GET["id"]."', 'datatableX')\">Close</button>&nbsp;
                        <button class='btn btn-primary' type='submit' 
                            onblur=\"shiftdataV2('document_data2.php?status=1&categoryid=".$_GET["categoryid"]."&empid=".$_GET["empid"]."&url=".$_GET["url"]."&id=".$_GET["id"]."', 'datatableX')\" onclick=\"const box = document.createElement('div');
                            box.style.cssText = 'position:fixed;top:20px;right:20px;background:#fff;padding:10px 20px;border-radius:10px;box-shadow:0 0 10px rgba(0,0,0,0.2);z-index:9999;display:flex;align-items:center;gap:10px;animation:fadeIn 0.5s ease;';
                            box.innerHTML = '<img src=assets/uploading.gif width=40 height=40><br><span style=font-family:sans-serif;font-size:16px;color:black>Uploading Image and Adding Data. Please wait...</span>';
                            document.body.appendChild(box); setTimeout(() => {  }, 20500);\">Update Document
                ...
                <div class='modal-footer' class='col-md-12' style='margin-top:15px;padding:5px;border-width:0px'>
                    ...
                    <button class='btn btn-primary' type='submit' onclick=\"const box = document.createElement('div');
                        box.style.cssText = 'position:fixed;top:20px;right:20px;background:#fff;padding:10px 20px;border-radius:10px;box-shadow:0 0 10px rgba(0,0,0,0.2);z-index:9999;display:flex;align-items:center;gap:10px;animation:fadeIn 0.5s ease;';
                        box.innerHTML = '<img src=assets/uploading.gif width=40 height=40><br><span style=font-family:sans-serif;font-size:16px;color:black>Uploading Image and Adding Data. Please wait...</span>';
                        document.body.appendChild(box); setTimeout(() => {  }, 20500);\">Upload and Save
                    </button>
                </div>
