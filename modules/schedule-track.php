        ...
        if($lts==1){
            ...
            if ($r5b->num_rows > 0) { while($r5c = $r5b -> fetch_assoc()) {
                ...
                if($pstatus!=0){
                    ...
                    echo"<div style='padding:5px'><a href='#' data-bs-toggle='modal' data-bs-target='#schedulemover1' class='btn btn-outline-primary btn-icon btn-icon-start' 
                        onclick=\"shiftdataV2('schedule-mover.php?t=1&smid=".$r5c["id"]."&days=$days&months=$months&years=$years&rdt=$datatarget&empid=$employeeid', 'schedulemover2')\" draggable='true' ondragstart='drag(event)' style='background-color:".$r5c["color"].";padding-left:10px;padding-right:10px;height:50px;width:100%;text-align:left;border-radius:10px'>
                        ...
                        <span style='font-size:8pt;color:white'>$stime1:$stime2 $sstat - $etime1:$etime2 $estat</span><br><span style='font-size:8pt;color:white'>$clientname</span>
                    </a></div>";
                }
            } }
        }
