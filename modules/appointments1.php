    ...
    echo"<div class='container'>
        <div class='page-title-container'>
            ...
            <div class='sidebar' id='templateSidebar' style='background-color:#111111;z-index:10010'>
              <header><h2>Add Shift Template</h2><button class='close-btn' id='close-template-sidebar'>&times;</button></header>
            ...
            <div class='sidebar' id='templateListSidebar' style='background-color:#111111;z-index:10000'>
              <header><h2>Shift Templates</h2><button class='close-btn' id='close-template-list-sidebar'>&times;</button></header>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Reuse existing Add Shift UI for template form (clone)
  // Supports both create + edit. If template_id is present -> update.
  function mountTemplateForm() {
    if (!templateFormMount) return null;
    if (templateFormMount.dataset.mounted === '1') {
      return templateFormMount.querySelector('form');
    }
    if (!addEventForm) {
    ...
    // Update heading inside template sidebar if needed
    ...
    // Clear id (templates use their own id)
    const idInput = clone.querySelector("input[name='id'], #event-id");
    if (idInput) idInput.value = '';

    // Add template_id hidden input (used for edit/update)
    let tplIdInput = clone.querySelector("input[name='template_id']");
    if (!tplIdInput) {
      tplIdInput = document.createElement('input');
      tplIdInput.type = 'hidden';
      tplIdInput.name = 'template_id';
      tplIdInput.value = '';
      clone.appendChild(tplIdInput);
    }

    templateFormMount.innerHTML = '';
    templateFormMount.appendChild(clone);
    templateFormMount.dataset.mounted = '1';

    // Submit: save/update template
    clone.addEventListener('submit', async (e) => {
      ...
      // The form uses hidden stime/etime populated by onchange on datetime-local.
      // Fallback to start/end if needed.
      if (!payload.stime && payload.start) payload.stime = payload.start;
      if (!payload.etime && payload.end) payload.etime = payload.end;

      // If template_id is present -> update, else create
      payload.action = payload.template_id ? 'template_update' : 'template_save';
      ...
      try {
        ...
        alert(payload.action === 'template_update' ? 'Template updated.' : 'Template saved.');
        closeAnySidebar(templateSidebar);

        // refresh list if it is open
        if (templateListSidebar && templateListSidebar.classList.contains('open')) {
          await loadTemplates();
        }
      } catch (err) {
        console.error(err);
  ...
  // Prefill template form for edit
  function openTemplateEditor(t) {
    const frm = mountTemplateForm();
    if (!frm) return;

    // set template_id
    const tplIdInput = frm.querySelector("input[name='template_id']");
    if (tplIdInput) tplIdInput.value = t.id || '';
    ...
    const setVal = (sel, val) => {
      const el = frm.querySelector(sel);
      if (!el) return;
      el.value = (val ?? '');
    ...
    setVal('#event-night', String(t.night ?? '0'));
    setVal('#event-pcolor', t.pcolor || t.color || '#CCCCCC');
    setVal('#event-admin_note', t.admin_note || '');
    setVal('#event-address', t.address || '');

    // datetime-local inputs
    setVal('#event-start', toInputLocal(t.stime));
    setVal('#event-end', toInputLocal(t.etime));
    // Ensure hidden stime/etime used by backend are updated
    setVal('#event-stime', toInputLocal(t.stime));
    setVal('#event-etime', toInputLocal(t.etime));
  ...
  async function loadTemplates() {
    ...
    try {
      const resp = await axios.get('calendar_backend_schedule.php', {
        params: { action: 'template_list', prjsrc: PRJ_ID },
      });
      ...
      // Render templates with Apply / Edit / Delete actions
      templateListContainer.innerHTML = rows
        .map((t) => {
          const st = t.stime || '';
          const en = t.etime || '';
          const rep = t.repeating || 'Once';
          const worker = t.employee || '';
          const safeJson = encodeURIComponent(JSON.stringify(t));
          return `
            <div class="tplRow" data-id="${t.id}" data-json="${safeJson}"
                 style="border:1px solid #333;border-radius:6px;padding:10px;margin:10px;background:#1a1a1a">
              <div style="font-weight:600">${worker}</div>
              <div style="font-size:12px">${st} - ${en}</div>
              <div style="font-size:12px">${rep}</div>
              <div style="display:flex;gap:8px;margin-top:8px">
                <button type="button" class="btn btn-success btn-sm tplApplyBtn" data-id="${t.id}">Apply</button>
                <button type="button" class="btn btn-primary btn-sm tplEditBtn" data-id="${t.id}">Edit</button>
                <button type="button" class="btn btn-danger btn-sm tplDeleteBtn" data-id="${t.id}">Delete</button>
              </div>
            </div>
          `;
        })
      ...
      // Apply
      templateListContainer.querySelectorAll('.tplApplyBtn').forEach((btn) => {
        btn.addEventListener('click', async (ev) => {
          ev.preventDefault();
          ev.stopPropagation();
          const tplId = btn.getAttribute('data-id');
          if (!tplId) return;
          if (!confirm('Add this template as a new shift?')) return;
          try {
            const r = await axios.post('calendar_backend_schedule.php', {
              action: 'template_apply',
              template_id: tplId,
              projectid: PRJ_ID,
            });
            if (!r?.data || r.data.status !== 'ok') throw new Error(r?.data?.message || 'Apply failed');
            if (window.calendar && typeof window.calendar.refetchEvents === 'function') {
              await window.calendar.refetchEvents();
            }
            alert('Template applied and shift added.');
          } catch (e) {
            console.error(e);
      ...
      // Delete
      templateListContainer.querySelectorAll('.tplDeleteBtn').forEach((btn) => {
        btn.addEventListener('click', async (ev) => {
          ev.preventDefault();
          ev.stopPropagation();
          const tplId = btn.getAttribute('data-id');
