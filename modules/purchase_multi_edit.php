<?php
    ...
    $pno=0;
    $s1x = "select * from purchase_voucher where proj_id='".$_GET["pid"]."' and purchased_from='".$_GET["pfrom"]."' and purchase_no='".$_GET["pno"]."' order by id asc limit 1";
    $r1x = $conn->query($s1x);
    ...
    echo"<div class='' style='width:100%'>";
        if($pno!=0){
            $s1 = "select * from purchase_voucher where proj_id='".$_GET["pid"]."' and purchased_from='".$_GET["pfrom"]."' and purchase_no='".$_GET["pno"]."' order by id asc";
            $r1 = $conn->query($s1);
