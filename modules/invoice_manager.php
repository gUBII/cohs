        ...
        if(isset($_GET["pid"])){
            echo"<form name='xcarbon' method='post' action='invoiceprocessor.php' target='invoicedataxxxx' enctype='multipart/form-data'>";
                    ...
                    if ($r->num_rows > 0) { while($rs = $r->fetch_assoc()) {
                        ...
                        echo"<div style='width:100%;padding:0px'>
                            <iframe name='invoicedataxxxx' src='./modules/invoices_addon.php?rid=$participantid&lid=$lid&sid=$invno2&pid=$invoiceid' scrolling='yes' style='border: 0px dashed #000000' border='0' frameborder='0' width='100%' height='450'>BROWSER NOT SUPPORTED</iframe>
                            <center>
                                <table><tr>
                                    <td><button type='button' class='btn btn-outline-primary btn-sm' data-bs-dismiss='offcanvas' >Close</button></td>
                                    <td><form method='POST' action='invoiceprocessor.php' target='dataprocessor' enctype='multipart/form-data' style='margin-top:8px;margin-bottom:5px'>
                                        <input type='hidden' name='processor' value='invoicegenerator'>
                                        <input type='hidden' name='invoice_no2' value='".$rs["invoice_no2"]."'>
                                        <input type='hidden' name='invoice_no' value='".$rs["invoice_no"]."'>
                                        <input type='hidden' name='ledger_id' value='".$rs["ledger_id"]."'>
            ...
            echo"<form name='xcarbon' method='post' action='invoiceprocessor.php' target='dataprocessor' enctype='multipart/form-data'>";
                if($_GET["cid"]==1001){
                    ...
                    <div class='row'>
                        <div class='col-md-12' style='padding:10px'><div style='width:100%;text-align:left'><b>Project Selection</b></div></div>
                        <div class='col-lg-12' style='padding:10px'><div style='width:100%;text-align:left'>
                            <label>Select Project Wise Client/Participant</label><br>";
                            echo"<select class='form-control' name='cname' required style='width:100%' id='clientProject'>";
                                if($_GET["clientid"]==0) echo"<option value=''>Select Client</option>";
                                /*
                                if(isset($_GET["cname"])){
                                ...
                                */
                                ...
                                if($mtype=="ADMIN") {
                                    if($_GET["clientid"]!=0){
                                        $p1x= "select * from project where clientid='".$_GET["clientid"]."' and status='1' order by name asc limit 1";
                                        $p1y = $conn->query($p1x);
                                        if ($p1y->num_rows > 0) { while($p1z = $p1y -> fetch_assoc()) {
                                            $r1x = "select * from uerp_user where id='".$p1z["clientid"]."' and status='1' order by username asc";
                                            $r1y = $conn->query($r1x);
                                            if ($r1y->num_rows > 0) { while($r1z = $r1y -> fetch_assoc()) {
                                                echo"<option value='".$p1z["id"]."'>".$r1z["username"]." ".$r1z["username2"]." @ ".$p1z["name"]."</option>";
                                            } }
                                        } }
                                    }
                                    $p1x= "select * from project where status='1' order by name asc";
                                }else{
                                    $p1x= "select * from project where teamleaderid='$userid' and status='1' OR leaderid='$userid' and status='1' OR caseid='$userid' and status='1' order by name asc";
                                }
                                $p1y = $conn->query($p1x);
                                if ($p1y->num_rows > 0) { while($p1z = $p1y -> fetch_assoc()) {
                                    $r1x = "select * from uerp_user where id='".$p1z["clientid"]."' and status='1' order by username asc";
                        ...
                        if($invoice_nox==0) $invoice_nox=1;
                        $invoice_number2="INV-$invoice_date-$invoice_nox";
                        
                        echo"<input type='hidden' class='form-control' id='invoice_no' name='invoice_no' value='0'>
                        <div class='col-md-12' style='padding:10px'><div style='width:100%;text-align:left'>
                            <label>Manual Invoice #</label><br>
                            <input type='text' class='form-control' id='invoice_no2' name='invoice_no2' value='$invoice_number2' required style='width:100%'>
                        </div></div>
                        
                        <div class='col-md-12' style='padding:10px'><div style='width:100%;text-align:left'><b>Select Data Sync Date Range</b></div></div>
                        
                        <div class='col-md-6' style='padding:10px'><div style='width:100%;text-align:left'>
                            <label>Date From</label><br><input type='date' class='form-control' name='invoice_date_from' value='$today' required style='width:100%'>
                        </div></div>
                        
                        <div class='col-md-6' style='padding:10px'><div style='width:100%;text-align:left'>
                            <label>Date To</label><br><input type='date' class='form-control' name='invoice_date_to' value='$today' required style='width:100%'>
                        </div></div>

                        <div class='modal-footer' class='col-md-12' style='margin-top:15px;padding:5px;border-width:0px'>
                            <button type='button' class='btn btn-outline-primary' data-bs-dismiss='offcanvas' onblur=\"shiftdataV2('shiftdataV2('invoice_table.php?cid=$cid&sheba=$sheba&utype=$utype&invdate1=".$_GET["invdate1"]."&invdate2=".$_GET["invdate2"]."&clientid=".$_GET["clientid"]."&url=create_invoice.php&id=5161', 'datatableX')\">Close</button>&nbsp;
                            ...
                            <button class='btn btn-primary' type='submit' onblur=\"shiftdataV2('invoice_table.php?cid=$cid&sheba=$sheba&utype=$utype&invdate1=".$_GET["invdate1"]."&invdate2=".$_GET["invdate2"]."&clientid=".$_GET["clientid"]."&url=create_invoice.php&id=5161', 'datatableX')\">Create Template</button>";
