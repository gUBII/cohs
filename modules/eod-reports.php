        ...
        if(!isset($_GET["editid"])){
            echo"<div class='table-responsive'>";
                ...
                echo"<table class='table table-striped table-bordered table-hover dataTables-example' >
                    <thead><tr><th nowrap>Date</th><th nowrap>Support W.</th><th nowrap>Participant</th><th>Shift</th><th>Activity</th><th>&nbsp;</th></tr></thead>
                    <tbody>";
                        $t=0;
                        ...
                        if($employeedata=="ALL" && $clientdata=="ALL"){
                            $ra1 = "select * from eod where eod_date>='$srcfdate' and eod_date<='$srctdate' and status='1' order by eod_date asc";
                        }else if($employeedata=="ALL" && $clientdata!="ALL"){
                            $ra1 = "select * from eod where clientid='$clientdata' and eod_date>='$srcfdate' and eod_date<='$srctdate' and status='1' order by eod_date asc";
                        }else if($employeedata!="ALL" && $clientdata=="ALL"){
                            $ra1 = "select * from eod where employeeid='$employeedata' and eod_date>='$srcfdate' and eod_date<='$srctdate' and status='1' order by eod_date asc";
                        }else{
                            $ra1 = "select * from eod where employeeid='$employeedata' and clientid='$clientdata' and eod_date>='$srcfdate' and eod_date<='$srctdate' and status='1' order by eod_date asc";
                        }
                        $rb1 = $conn->query($ra1);
                        if ($rb1->num_rows > 0) { while($rab1 = $rb1->fetch_assoc()) {
                            ...
                            echo"<tr class='gradeX'>
                                <td nowrap>ID: ".$rab1["eod_date"]."-".$rab1["id"]."<br>$eod_date
                                </td>
                                <td>$employeename</td>
                                <td>$clientname</td>
                                <td>".$rab1["shiftid"]."</td>
                                <td>";
                                    if($rab1["activityid"]=="Array") $activityid="-";
                                    else $activityid=$rab1["activityid"];
                                    echo"$activityid";
                                echo"</td>
                                <td align=center>
                                    <div class='d-inline-block datatable-export' data-datatable='#datatableRows'>
                                        ...
                                        <div class='dropdown-menu shadow dropdown-menu-end' style='padding:10px'>";
                                            echo"<a class='dropdown-item'data-bs-toggle='offcanvas' data-bs-target='#offcanvasTop2' aria-controls='offcanvasTop2' onmouseover=\"shiftdataV2('eod_detail_view.php?postid=".$rab1["id"]."', 'offcanvasTop2')\" href='#' data-bs-toggle='tooltip' data-bs-placement='bottom' title='View'><i class='fa fa-eye'></i> View</a>";
                                            if($mtype=="ADMIN"){
                                                echo"<hr><a style='padding:27px' href='index.php?employeedata=".$_GET["employeedata"]."&clientdata=".$_GET["clientdata"]."&srcfdate=".$_GET["srcfdate"]."&srctdate=".$_GET["srctdate"]."&url=eod-reports.php&id=0&sheba=$sheba&editid=".$rab1["id"]."'><i class='fa fa-edit'></i> Edit</a>";
                                                // echo"<hr><a style='padding:27px' href='' data-bs-placement='bottom' title='Delete'><i class='fa fa-trash'></i> Delete</a>";
                                            }
                                            echo"<hr><a style='padding:27px' href='excel/export_excel.php?employeedata=".$rab1["employeeid"]."&clientdata=".$rab1["clientid"]."&date_from=$dateft&date_to=$dateft&kroyee=eod-csv-reports&pointer=PDF' target='dataprocessor' data-bs-placement='bottom' title='Download'><i class='fa fa-download'></i> Download</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>"; 
