    ...
    <div class='offcanvas-body'>
        <form method='post' action='dataprocessor_1.php' target='dataprocessor' enctype='multipart/form-data' id='loginForm' class='tooltip-end-bottom' novalidate >";
            if(isset($_GET["mid"]) && $_GET["mid"]!=0){
                    ...
                    $s1x = "select * from sub_ledger where id='".$_GET["mid"]."' order by id asc limit 1";
                    $r1x = $conn->query($s1x);
                    if ($r1x->num_rows > 0) { while($rs1x = $r1x->fetch_assoc()) {
                        ...
                        <div class='col-md-12' style='margin-top:15px'>
                            ...
                            <select class='form-select' name='ledger_id' required='' >";
                                $s1z = "select * from accounts_ledger where id='".$rs1x["ledger_id"]."' order by id asc limit 1";
                                $r1z = $conn->query($s1z);
                                if ($r1z->num_rows > 0) { while($rs1z = $r1z->fetch_assoc()) {
                                    echo"<option value='".$rs1z["id"]."'>".$rs1z["ledger_name"]."</option>";
                                } }
                                $s1y = "select * from accounts_ledger where status='ON' order by ledger_name asc";
                                $r1y = $conn->query($s1y);
                                if ($r1y->num_rows > 0) { while($rs1y = $r1y->fetch_assoc()) {
                                    echo"<option value='".$rs1y["id"]."'>".$rs1y["ledger_name"]."</option>";
                                } }
                            echo"</select>
                        </div>
                        <div class='mb-3'><label class='form-label'>Sub Account Name</label><input class='form-control' type='text' name='sub_ledger' placeholder='Type here' value='".$rs1x["sub_ledger"]."' ></div>
                        ...
                        <div class='modal-footer' class='col-md-12' style='margin-top:15px;padding:5px;border-width:0px'>
                            <button type='button' class='btn btn-outline-primary' data-bs-dismiss='offcanvas' onclick=\"shiftdataV2('ledger_sub_accounts_data.php?cid=4&sheba=sub_ledger', 'datatableX')\">Close</button>&nbsp;
                            <button class='btn btn-primary' type='submit' data-bs-dismiss='offcanvas' onblur=\"shiftdataV2('ledger_sub_accounts_data.php?cid=4&sheba=sub_ledger', 'datatableX')\">Update</button>
                        </div>";
                    } }
                    ...
                    $cdate=time();
                    $cdate=date("Y-m-d",$cdate);
                    echo"<input type=hidden name='processor' value='new_sub_account'>
                    ...
                    <div class='mb-3'><label class='form-label'>Sub Account Name</label><input class='form-control' type='text' name='sub_ledger' placeholder='Type here' value='' ></div>
                    <div class='mb-3'><label class='form-label'>Opening Balance</label><input class='form-control' type='text' name='opening_balance' placeholder='Type here' value='' ></div>
                    ...
                    <div class='modal-footer' class='col-md-12' style='margin-top:15px;padding:5px;border-width:0px'>
                        <button type='button' class='btn btn-outline-primary' data-bs-dismiss='offcanvas' onclick=\"shiftdataV2('ledger_sub_accounts_data.php?cid=4&sheba=sub_ledger', 'datatableX')\">Close</button>&nbsp;
                        <button class='btn btn-primary' type='submit' data-bs-dismiss='offcanvas' onblur=\"shiftdataV2('ledger_sub_accounts_data.php?cid=4&sheba=sub_ledger', 'datatableX')\">Save</button>
                    </div>";
