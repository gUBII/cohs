                ...
                <div class='row'>
                    ...
                    <div class='col-12 col-md-8 d-flex align-items-start justify-content-end'>
                        ...
                        <form class='m-t' role='form' method='POST' name='wcopy' action='dataprocessor.php' id='schedule-form' target='dataprocessor' enctype='multipart/form-data'>
                            ...
                            <a href='index.php?url=appointments_allocated.php&viewstatus=1' class='btn btn-outline-secondary btn-icon btn-icon-start w-md-auto btn-sm' style='margin-right:10px' data-bs-delay='0' data-bs-offset='0,3' data-bs-toggle='tooltip' data-bs-placement='bottom' title='View All Clients` Roster'><i data-acorn-icon='eye'></i></a>
                        ...
                        <div class='modal fade' id='budgetshift' tabindex='-1' role='dialog' aria-hidden='true'>
                            <div class='modal-dialog modal-semi-full modal-dialog-centered'>
                                <div class='modal-content'>
                                    ...
                                    <div class='modal-body'>
                                        <iframe src='datarpocessor.php?processor=cloneweek&id=1&url=appointments_allocated.php&employeeid=ALL' name='budgetmanagery' scrolling='yes' style='border: 0px dashed #000000' border='0' frameborder='0' width='100%' height='550'>BROWSER NOT SUPPORTED</iframe>
            ...
            if(isset($viewpoints)){
                ...
                echo"<div class='col-12 col-md-12'>
                    ...
                    <form method='get' action='index.php'>
                        <input type=hidden name=url value='appointments_allocated.php'><input type=hidden name=viewstatus value='2'>
                        <table width=100% ><tr>
                            <td>
...
function mountTemplateForm() {
  ...
  clone.addEventListener('submit', async (e) => {
    ...
    try {
      const resp = await axios.post('calendar_backend_schedule_allocated.php', payload);
...
async function openTemplateEditor(templateId) {
  ...
  try {
    const resp = await axios.get('calendar_backend_schedule_allocated.php', {
  ...
  try {
    ...
    templateListContainer.querySelectorAll('.tplDelBtn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          const r = await axios.post('calendar_backend_schedule_allocated.php', { action: 'template_delete', template_id: id });
...
if (closeTemplateListBtn) closeTemplateListBtn.addEventListener('click', () => closeAnySidebar(templateListSidebar));
  ...
  function addHoursToIsoLocal(isoLocal, hours){
    ...
    function ensureModalActionButtons() {
        ...
        if (!acceptBtn.dataset.bound) {
            ...
            acceptBtn.addEventListener('click', async () => {
                ...
                const id = selectedEvent.extendedProps.idx || selectedEvent.id;
                try {
                const resp = await axios.post('calendar_backend_schedule_allocated.php', { action: 'accept', id });
        ...
        if (!requestBtn.dataset.bound) {
            ...
            try {
              const resp = await axios.post('calendar_backend_schedule_allocated.php', { action: 'request_time', id, clockin_request: ci, clockout_request: co });
        ...
        if (confirmBtn && !confirmBtn.dataset.bound) {
          ...
          confirmBtn.addEventListener('click', async () => {
            ...
            try {
              const resp = await axios.post('calendar_backend_schedule_allocated.php', {
                action: 'cancel',
                id: id,
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: async function (fetchInfo, successCallback, failureCallback) {
            try {
                const response = await axios.get('calendar_backend_schedule_allocated.php', { 
  ...
  <?php if($designationy=="13" OR $designationy=="29" OR $teamleaderid==$userid OR $designationy=="20") { ?>
      ...
      if (addEventForm) addEventForm.addEventListener('submit', async function (e) {
        ...
        try {
          const response = await axios.post('calendar_backend_schedule_allocated.php', newEvent);
          ...
          if (confirm('Do you want to save this new shift as a template also?')) {
            try {
              ...
              const tplResp = await axios.post('calendar_backend_schedule_allocated.php', tplPayload);
      ...
      if (editEventForm) editEventForm.addEventListener('submit', async function (e) {
        ...
        try {
          const response = await axios.post('calendar_backend_schedule_allocated.php', editEvent);
      ...
      if (deleteEventBtn) deleteEventBtn.addEventListener('click', async function () {
        if (confirm('Are you sure you want to delete this Shift?')) {
          await axios.post('calendar_backend_schedule_allocated.php', { id: selectedEvent.id, delete: true });
...
(function() {
  ...
  async function loadRepeatFilters() {
    try {
      const [cRes, eRes] = await Promise.all([
        axios.get('calendar_backend_schedule_allocated.php', { params: { action: 'clients_list' } }),
        axios.get('calendar_backend_schedule_allocated.php', { params: { action: 'employees_list' } })
      ]);
  ...
  async function loadRepeatSourceTable() {
    ...
    try {
      const res = await axios.get('calendar_backend_schedule_allocated.php', {
