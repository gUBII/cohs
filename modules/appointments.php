...
<?php
    ...
    if(isset($_GET["accepted"])) $accepteds=$_GET["accepted"];
    else $accepteds=0;
        ...
        <div class='page-title-container'>
                ...
                <div class='row'>
                    ...
                    <div class='col-12 col-md-8 d-flex align-items-start justify-content-end'>
                        ...
                        <form class='m-t' role='form' method='POST' name='wcopy' action='dataprocessor.php' id='schedule-form' target='dataprocessor' enctype='multipart/form-data'>
                            ...
                            <a href='index.php?url=appointments.php&viewstatus=1' class='btn btn-outline-secondary btn-icon btn-icon-start w-md-auto btn-sm' style='margin-right:10px' data-bs-delay='0' data-bs-offset='0,3' data-bs-toggle='tooltip' data-bs-placement='bottom' title='View All Clients` Roster'><i data-acorn-icon='eye'></i></a>
            ...
            echo"<div id='calendar-container'>
                ...
                if(isset($_GET["prjsrc"])){
                    ...
                    if($accepteds==1) echo"<h2 style='color:orange'>Allocated Appointments</h2>";
                    if($accepteds==0) echo"<h2 style='color:orange'>UnAllocated Appointments</h2>";
                    if($accepteds==2) echo"<h2 style='color:orange'>Tentative Appointments</h2>";
                    
                    echo"<a href='index.php?url=appointments.php&viewstatus=".$_GET["viewstatus"]."&prjsrc=".$_GET["prjsrc"]."&accepted=1' class='btn btn-outline-info btn-sm' style='margin-right:5px' data-bs-delay='0' data-bs-offset='0,3' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Allocated Shifts'>Allocated</a>
                    <a href='index.php?url=appointments.php&viewstatus=".$_GET["viewstatus"]."&prjsrc=".$_GET["prjsrc"]."&accepted=0' class='btn btn-outline-info btn-sm' style='margin-right:5px' data-bs-delay='0' data-bs-offset='0,3' data-bs-toggle='tooltip' data-bs-placement='bottom' title='UnAllocated Shifts'>Unallocated</a>";
                    // <a href='index.php?url=appointments.php&viewstatus=".$_GET["viewstatus"]."&prjsrc=".$_GET["prjsrc"]."&accepted=2' class='btn btn-outline-info btn-sm' style='margin-right:5px' data-bs-delay='0' data-bs-offset='0,3' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Allocated Shifts'>Tentitive</a>
                    echo"<a href='index.php?url=schedule_jobs_updated.php&viewstatus=".$_GET["viewstatus"]."&prjsrc=".$_GET["prjsrc"]."&accepted=$accepteds' class='btn btn-outline-info btn-sm' style='margin-right:5px' data-bs-delay='0' data-bs-offset='0,3' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Allocated Shifts'>Weekly Planner</a>
                                            ...
                                            if(isset($_GET["prjsrc"])){
                                                ...
                                                echo"<div class='col-12' style='text-align:left'>
                                                    ...
                                                    <select class='form-control' id='event-itemnumber' name='itemnumber' required style='width:100%;margin-bottom:10px'>
                                                        ...
                                                        echo"<option value='' disabled><strong>More Item Number</option>";
...
<?php
  $acceptedParam = 0;
  if (isset($_GET['accepted']) && ($_GET['accepted'] === '0' || $_GET['accepted'] === '1')) {
      $acceptedParam = (int)$_GET['accepted']; // 0 or 1
  }
?>
<script>
    const PRJ_ID = <?php echo $projectidx; ?>;
    const ACCEPTED_FILTER = <?php echo json_encode($acceptedParam); ?>; // null, 0, or 1
</script>
...
if (closeTemplateListBtn) closeTemplateListBtn.addEventListener('click', () => closeAnySidebar(templateListSidebar));
  ...
  function addHoursToIsoLocal(isoLocal, hours){
    ...
    // ---------- Ensure Activity Log Modal exists ----------
    function ensureActivityLogModal(){
        if(document.getElementById('activityLogModal')) return;
        const html = `
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="d-flex align-items-center gap-2 mb-2">
                                <div class="text-muted small" id="activityLogMeta">—</div>
                                <div class="ms-auto text-muted small" id="activityLogStatus"></div>
                            </div>
                                ...
                                <table class="table table-sm table-striped mb-0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="white-space:nowrap">Tran Type</th>
    ...
    async function openActivityLogModal(){
      ...
      meta.textContent = id ? `Shift/Allocation ID: ${id}` : '—';
      status.textContent = 'Loading...';
      tbody.innerHTML = `<tr><td colspan="6" class="text-muted">Loading...</td></tr>`;

      try {
        const resp = await axios.post('calendar_backend_schedule.php', { action: 'activity_log', id, limit: 200 });
        if (!resp?.data || resp.data.status !== 'ok') throw new Error(resp?.data?.message || 'Failed');

        const rows = Array.isArray(resp.data.rows) ? resp.data.rows : [];
        if (rows.length === 0) {
          tbody.innerHTML = `<tr><td colspan="6" class="text-muted">No activity records found.</td></tr>`;
        } else {
          tbody.innerHTML = rows.map(r => {
            ...
            return `
              <tr>
                ...
                <td style="white-space:nowrap">${esc(r.tran_type)}</td>
        ...
        console.error(e);
        status.textContent = 'Audit Trail.';
        tbody.innerHTML = `<tr><td colspan="6" class="text-danger">no activity log found.</td></tr>`;
      }

      new bootstrap.Modal(document.getElementById('activityLogModal'), {backdrop:'static', keyboard:true}).show();
    }
    ...
    function ensureModalActionButtons() {
        ...
        // Add Activity Log button before/beside "Request Change"
        let activityBtn = document.getElementById('activityLogBtn');
        if (!activityBtn) {
            activityBtn = document.createElement('button');
            activityBtn.type = 'button';
            activityBtn.id = 'activityLogBtn';
            activityBtn.className = 'btn btn-outline-info btn-sm';
            activityBtn.textContent = 'Activity Log';
            // Place it right before Request Change
            if (requestBtn && requestBtn.parentNode) requestBtn.parentNode.insertBefore(activityBtn, requestBtn);
            else footer.prepend(activityBtn);
        }
        ...
        if (activityBtn && !activityBtn.dataset.bound) {
            ...
            activityBtn.addEventListener('click', async () => {
                if (!selectedEvent) return;
                $('#eventModal').modal('hide');
                // give bootstrap a moment to complete closing animation
                setTimeout(() => openActivityLogModal(), 250);
            });
        }
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDidMount: function(info) {
          ...
          if (cancelled) {
            allocatedColor = '#E30D0D'; // grey
            textColor = '#ffffff';
            textCancelColor = '#ffffff';
          } else if (unaccepted) {
            ...
            if (info.event.extendedProps.cancelled_status === "Cancellation Reason") {
                buttonsHtml += `<button style="width:100%;font-size:8pt;margin-bottom:5px" class="btn btn-info btn-sm">
            ...
            info.el.addEventListener('mouseenter', () => {
                if (cancelled) {
                    info.el.style.filter = 'brightness(95%)';
                } else {
            ...
            info.el.addEventListener('mouseleave', () => {
                info.el.style.filter = cancelled ? '${allocatedColor}' : 'brightness(100%)';
        ...
        events: async function (fetchInfo, successCallback, failureCallback) {
            try {
                const params = { prjsrc: PRJ_ID, t: Date.now() };
                if (ACCEPTED_FILTER !== null) params.accepted = ACCEPTED_FILTER; // 0 or 1
        
                const response = await axios.get('calendar_backend_schedule.php', { params });
        ...
        try {
          ...
          if (confirm('Do you want to save this new shift as a template also?')) {
            try {
              ...
              const tplResp = await axios.post('calendar_backend_schedule.php', tplPayload);
