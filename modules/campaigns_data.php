                                ...
                                <td class='text-alternate'>
                                    <div class='d-inline-block datatable-export' data-datatable='#datatableRows' style='background-color:grey;padding:5px;width:30px;text-align:center;border-radius:5px'>
                                        ...
                                        <div class='dropdown-menu shadow dropdown-menu-end' onmouseup=\"shiftdataV2('leaves_data.php.php?cid=$cid&sheba=$sheba', 'datatableX')\">
                                            <a class='dropdown-item' style='cursor: pointer' data-bs-toggle='offcanvas' data-bs-target='#offcanvasRight' aria-controls='offcanvasRight' onmouseover=\"shiftdataV2('global_manager.php?dataid=".$rab1["id"]."&cid=$cid&sheba=$sheba&sourceid=Campaign&ctitle=Edit/Approve Campaings', 'offcanvasRight')\">Edit</a>";
                                            if($rab1["status"]=="ON") echo"<a class='dropdown-item' href='dataprocessor_1.php?processor=StatusSuspend&status=OFF&tid=campaigns&pid=".$rab1["id"]."' target='dataprocessor' onblur=\"shiftdataV2('campaigns_data.php?cid=".$_GET["cid"]."&sheba=".$_GET["sheba"]."&utype=".$_GET["utype"]."&sourceid=campaigns&url=".$_GET["url"]."&id=".$_GET["id"]."', 'datatableX')\">Suspend</a>";
                                            else echo"<a class='dropdown-item' href='dataprocessor_1.php?processor=StatusSuspend&status=ON&tid=campaigns&pid=".$rab1["id"]."' target='dataprocessor' onblur=\"shiftdataV2('campaigns_data.php?cid=".$_GET["cid"]."&sheba=".$_GET["sheba"]."&utype=".$_GET["utype"]."&sourceid=campaigns&url=".$_GET["url"]."&id=".$_GET["id"]."', 'datatableX')\">Activate</a>";
                                            // echo"<a class='dropdown-item' style='cursor: pointer' data-bs-toggle='offcanvas' data-bs-target='#offcanvasTop' aria-controls='offcanvasTop' onblur=\"shiftdataV2('campaigns_data.php?cid=".$_GET["cid"]."&sheba=".$_GET["sheba"]."&utype=".$_GET["utype"]."&sourceid=campaigns&url=".$_GET["url"]."&id=".$_GET["id"]."', 'datatableX')\">Delete</a>";
                                        echo"</div>
                                    </div>
                                </td>
                                <td>&nbsp;</td>
