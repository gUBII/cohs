...
<?php
    ...
    echo"<div class='row'>
        ...
        <div class='col-12 col-md-7 d-flex align-items-start justify-content-end'>";
            ...
            if($_GET["clientid"]!=0){
                echo"<a href='index.php?url=paid_invoices.php&id=5163&selection=INCOME&statusupdate=2&poinz=2&lid_date_from=$lid_date_from&lid_date_to=$lid_date_to&lid=800000032&clientid=$clientid&refby=$refby&clientid=".$_GET["clientid"]."' class='btn btn-outline-tertiary btn-icon btn-icon-start w-md-auto btn-sm' type='button'>
                    <i data-acorn-icon='eye'></i>&nbsp;&nbsp;<span>Paid</span>
                </a>";
            }else{
                echo"<a href='index.php?url=paid_invoices.php&id=5163&selection=INCOME&statusupdate=2&poinz=2&lid_date_from=$yr-02-01&lid_date_to=$currentdate&lid=800000032&clientid=$clientid&refby=$refby' class='btn btn-outline-tertiary btn-icon btn-icon-start w-md-auto btn-sm' type='button'>
                    <i data-acorn-icon='eye'></i>&nbsp;&nbsp;<span>Paid</span>
                </a>";
            }
        echo"</div>                  
        ...
        <form method='get' action='index.php' target='_top' enctype='multipart/form-data'>
            ...
            <table style='width:100%'><tr>
                    ...
                    <select class='form-control' name='lid'>";
                        if($mtype=="CLIENT"){
                            ...
                            if(isset($_GET["lid"])){
                                ...
                                $r1xx = $conn->query($r1);
                                if ($r1xx->num_rows > 0) { while($r1yy = $r1xx -> fetch_assoc()) {
                                    if($r1yy["id"]==$_GET["lid"]) echo"<option value='".$r1yy["id"]."' selected>".$r1yy["ledger_name"]."</option>";
                                    else echo"<option value='".$r1yy["id"]."'>".$r1yy["ledger_name"]."</option>";    
                                } }
                            }else{
                                $r1 = "select * from accounts_ledger where balance_type='INCOME' order by ledger_name asc";
                                $r1xx = $conn->query($r1);
                                if ($r1xx->num_rows > 0) { while($r1yy = $r1xx -> fetch_assoc()) {
                                    if($r1yy["id"]=="800000032") echo"<option value='".$r1yy["id"]."' selected>".$r1yy["id"]." - ".$r1yy["ledger_name"]."</option>";
                                    else echo"<option value='".$r1yy["id"]."'>".$r1yy["id"]." - ".$r1yy["ledger_name"]."</option>";    
                                } }
                            }
                        }
                    echo"</select>
                ...
                <td align=left style='padding:10px'>
                    ...
                    <select class='form-control' name='clientid' required style='width:100%'><option value='ALL'>ALL</option>";
                        ...
                        if($mtype!="CLIENT"){
                            ...
                            if ($r1y->num_rows > 0) { while($r1z = $r1y -> fetch_assoc()) {
                                if(isset($_GET["clientid"]) && $_GET["clientid"]!="ALL" && $r1z["id"]==$_GET["clientid"]) echo"<option value='".$r1z["id"]."' selected>".$r1z["username"]." ".$r1z["username2"]."</option>";
                                else echo"<option value='".$r1z["id"]."'>".$r1z["username"]." ".$r1z["username2"]."</option>";
                            } }
                        }else{
    ...
    <div class='col-lg-12' style='padding:20px'>";
        ...
        if($mtype!="CLIENT"){
            echo"<a href='modules/myob_invoice_export.php?xid=ALL' target='dataprocessor' id='downloadBtn' class='btn btn-outline-primary btn-sm' >Download MYOB File</a>&nbsp;&nbsp;";
            // echo"<button id='editSelectedBtn' class='btn btn-outline-warning btn-sm'>Edit Selected</button>";
            // echo"<button id='deleteSelectedBtn' class='btn btn-outline-danger btn-sm'>Delete Selected</button>";
            echo"<button id='markAsPaidBtn' class='btn btn-success btn-sm'>Move Selected to Paid</button><hr>";
        ...
        echo"<table id='invoiceTable' class='display' style='width:100%'>
            ...
            <tbody>";
                ...
                if(isset($userid)){
                    ...
                    if($lid>=0){
                        $ts=0;
                        if($clientid=="ALL") $s = "select * from receipt_voucher where ledger_id='$lid' and receipt_date>='$lid_date1' and receipt_date<='$lid_date2' and invoice_no2<>'0' and paid='1' group by invoice_no2";
                        else $s = "select * from receipt_voucher where received_from='$clientid' and ledger_id='$lid' and receipt_date>='$lid_date1' and receipt_date<='$lid_date2' and invoice_no2<>'0' and paid='1' group by invoice_no2";
                        $r = $conn->query($s);
                        if ($r->num_rows > 0) { while($rs = $r->fetch_assoc()) {
                            ...
                            $totpaid=0;
                            // $s1 = "select * from receipt_voucher where invoice_no2='".$rs["invoice_no2"]."' and id='".$rs["id"]."' and paid='1' order by id desc";
                            $s1 = "select * from receipt_voucher where invoice_no2='".$rs["invoice_no2"]."' and ledger_id='".$rs["ledger_id"]."' and paid='1' order by ledger_id desc";
                            $r1 = $conn->query($s1);
                            ...
                            $cardid="";
                            $r1x = "select * from ndis_card_number where id='".$rs["invoice_no"]."' order by id asc limit 1";
                            $r1y = $conn->query($r1x);
                            if ($r1y->num_rows > 0) { while($r1z = $r1y -> fetch_assoc()) {
                                $cardid="".$r1z["card_number"]." - ".$r1z["referrer"]."";
                            } }
                            ...
                            if($_GET["selection"]=="INCOME"){
                                ...
                                echo"<tr class='' style='width:100%'>
                                    <td><input type='checkbox' class='row-checkbox' name='selected_ids[]' value='".$rs["invoice_no2"]."'></td>
                                    <td class='text-alternate' align='left' style='width:150px;'>$cardid</td>
                                    <td class='text-alternate' align='left' style='width:150px;'>$date</td>
                                    <td class='text-alternate' align='left' style='width:150px;'>ADDON-".$rs["randomid"]."</td>
                                    ...
                                    <td class='text-alternate' align='center' style='width:100px;'>";
                                        if($mtype!="CLIENT"){
                                            ...
                                            <div class='dropdown-menu shadow dropdown-menu-end' onmouseup=\"shiftdataV2('ledger_pending_invoices.php?cid=$cid&sheba=$sheba&utype=$utype&selection=$selection&statusupdate=1&poinz=2&lid_date_from=$lid_date_from&lid_date_to=$lid_date_to&lid=$lid&clientid=$clientid', 'datatableX')\">
                                                ...
                                                echo"<a class='dropdown-item' style='cursor: pointer' href='modules/ledger_invoice_csv.php?pid=".$rs["id"]."-1&invoiceid=".$rs["invoice_no2"]."&ledgerid=".$rs["ledger_id"]."' target='dataprocessor'>Generate NDIS File</a>
                                                <a class='dropdown-item' style='cursor: pointer' href='modules/myob_invoice_export.php?pid=".$rs["id"]."-1' target='dataprocessor'>Generate MYOB File</a>";
                                                // echo"<a class='dropdown-item' style='cursor: pointer' href='invoiceprocessor.php?processor=invoicepaymentgenerator&lid=".$_GET["lid"]."&sid=".$rs["invoice_no2"]."&clientid=".$rs["received_from"]."&dfrom=".$_GET["lid_date_from"]."&dto=".$_GET["lid_date_to"]."&id=5162&url=unpaid_invoices.php&utype=".$_GET["utype"]."&selection=".$_GET["selection"]."&statusupdate=".$_GET["statusupdate"]."&poinz=".$_GET["poinz"]."&cid=".$_GET["cid"]."&sheba=".$_GET["sheba"]."&refby=".$_GET["sheba"]."' target='dataprocessor'>Mark as Paid</a>";
                                                // <a class='dropdown-item' style='cursor: pointer' data-bs-toggle='offcanvas' data-bs-target='#offcanvasTop' aria-controls='offcanvasTop' onclick=\"shiftdataV2('../delete_records.php?tbl=receipt_voucher&proj_id=".$rs["proj_id"]."&delid=".$rs["id"]."&inv_id=".$rs["invoice_no2"]."', 'offcanvasTop')\">Delete</a>";
                                            echo"</div>";
                                        }
                                    echo"</td>
                                </tr>";
