<?php
    ...
    echo"<div class='row'>
        <div class='col-12 col-md-5' style='padding-bottom:10px'>
            <h1 class='mb-0 pb-0 display-4' id='title'>H-Care Services Offered</h1>
        ...
        <div class='col-12 col-md-7 d-flex align-items-start justify-content-end'>
            ...
            <button class='btn btn-outline-primary btn-icon btn-icon-start w-md-auto btn-sm' type='button' data-bs-toggle='offcanvas' data-bs-target='#offcanvasRight' aria-controls='offcanvasRight' onmouseover=\"shiftdataV2('chart_of_accounts_manager.php?cid=$cid&sheba=$sheba&ctitle=$title', 'offcanvasRight')\">
        ...
        <div class='data-table-responsive-wrapper'>
            <table class='data-table data-table-pagination data-table-standard responsive nowrap stripe hover' id='myTable' style='width:100%'>
                <thead><tr>
                    ...
                    <th class='text-muted text-small text-uppercase'>Sub Services</th>
                ...
                <tbody class='list-unstyled' id='page_list'>";
                    ...
                    if ($ms11->num_rows > 0) { while($ms111 = $ms11 -> fetch_assoc()) {
                        ...
                        echo"<tr class='zoom' id='".$ms111["id"]."' style='width:100%'>
                            <td class='text-alternate'><i data-acorn-icon='sort' class='icon' data-acorn-size='10'></i></td>
                            
                            <td class='text-alternate'>".$ms111["name"]."</td>
                            <td class='text-alternate'>-</td>
                            <td style='width:30px;text-align:right'><div class='form-check form-switch' data-bs-toggle='tooltip' data-bs-placement='left' title='Module Updated'>
                                <form name='saas$rand' method=post action='system/dataprocessor.php' target='dataprocessor'>
                                    ...
                                    if($ms111["dashboard"]==1) echo"<input class='form-check-input' type='checkbox' id='flexSwitchCheckChecked' checked name='switch' value='0'/>";
                                    else echo"<input class='form-check-input' type='checkbox' id='flexSwitchCheckChecked' name='switch' value='1'/>";
                                echo"</form>
                            </div></td>
                            <td class='text-alternate'>".$ms111["status"]."</td>
                        </tr>";
                        $ms2 = "select * from service_offered where parent='$lastid' order by name asc";
                        $ms22 = $conn->query($ms2);
                        if ($ms22->num_rows > 0) { while($ms222 = $ms22 -> fetch_assoc()) {
                            ...
                            echo"<tr class='zoom' id='".$ms222["id"]."' style='width:100%'>
                                <td class='text-alternate'><i data-acorn-icon='sort' class='icon' data-acorn-size='10'></i></td>
                                <td class='text-alternate'>-</td>
                                <td class='text-alternate'>".$ms222["name"]."</td>
                                <td style='width:30px;text-align:right'><div class='form-check form-switch' data-bs-toggle='tooltip' data-bs-placement='left' title='Module Updated'>
                                    <form name='saas$rand' method=post action='system/dataprocessor.php' target='dataprocessor'>
                                        <input type='hidden' name='processor' value='solutionswitch'><input type='hidden' name='id' value='".$ms222["id"]."'><input type='hidden' name='switchx' value=''>";
                                        if($ms222["dashboard"]==1) echo"<input class='form-check-input' type='checkbox' id='flexSwitchCheckChecked' checked name='switch' value='0'/>";
                                        else echo"<input class='form-check-input' type='checkbox' id='flexSwitchCheckChecked' name='switch' value='1'/>";
                                    echo"</form>
                                </div></td>
                                <td class='text-alternate'>".$ms222["status"]."</td>
