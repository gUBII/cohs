            ...
            <div id='calendar-container'>
                <div class='row'>
                    ...
                    <div class='col-12 col-md-7 d-flex align-items-start justify-content-end'>
                        ...
                        echo"<button type='button' class='btn btn-outline-primary btn-sm' id='repeatBtn'>Repeat</button>";
...
// ---- Create Repeat Settings Modal (Modal #1) if missing ----
function ensureRepeatSettingsModal(){
  if (document.getElementById('repeatSettingsModal')) return;

  const html = `
    <div class="modal fade" id="repeatSettingsModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Repeat Shifts</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>

          <div class="modal-body">
            <label class="form-label">Select Target Week</label>
            <input type="week" id="weekfrom" class="form-control" name="weekfrom" style="margin-bottom:10px">

            <label class="form-label">Repeat Frequency</label>
            <select class="form-control" id="repeatFrequency" style="margin-bottom:10px">
              <option value="Week Repeat">Week Repeat</option>
              <option value="Fortnight Repeat">Fortnight Repeat</option>
              <option value="3 Weeks Repeat">Every 3 Weeks</option>
              <option value="4 Weeks Repeat">Every 4 Weeks</option>
            ...
            <div class="form-text">
              Next will load shifts where <code>repeating</code> equals the selected frequency, and
              where <code>status=1</code>, <code>cancelled=0</code>, <code>accepted=1</code>, and IDs are not 0.
            </div>
          </div>

          <div class="modal-footer">
            <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button>
            <button type="button" class="btn btn-primary" id="repeatNextBtn">Next</button>
          </div>
        </div>
      </div>
    </div>
...
// ---- Create Repeat List Modal (Modal #2) if missing ----
function ensureRepeatListModal(){
  if (document.getElementById('repeatListModal')) return;

  const html = `
    <div class="modal fade" id="repeatListModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Select Shifts to Repeat</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>

          <div class="modal-body">
            <div class="d-flex justify-content-between align-items-center mb-2">
              <div class="small text-muted" id="repeatListInfo"></div>
              <div>
                <button class="btn btn-outline-secondary btn-sm" id="repeatSelectAll" type="button">Select All</button>
                <button class="btn btn-outline-secondary btn-sm" id="repeatSelectNone" type="button">Select None</button>
              </div>
            </div>

            <table id="repeatTable" class="table table-striped table-bordered w-100">
              <thead>
                <tr>
                  <th style="width:40px"><input type="checkbox" id="repeatCheckAll"></th>
                  <th>ID</th>
                  <th>Client</th>
                  <th>Worker</th>
                  <th>Start</th>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button>
            <button type="button" class="btn btn-success" id="copyRepeatBtn">Copy Repeat Records</button>
          </div>
        </div>
      </div>
    </div>
...
// ---- ISO week helpers for default next week ----
...
function setWeekInputToNextWeek() {
  const inp = document.getElementById('weekfrom');
  if (!inp) return;
  ...
  inp.value = `${year}-W${String(week).padStart(2,'0')}`;
}

// ---- DataTable ----
let repeatDT = null;

function initRepeatTable(repeatingValue) {
  ensureRepeatListModal();

  const url = 'repeat_backend.php?action=list&repeating=' + encodeURIComponent(repeatingValue);

  if (repeatDT) {
    repeatDT.ajax.url(url).load();
    return;
  }

  repeatDT = $('#repeatTable').DataTable({
    processing: true,
    serverSide: false,
    pageLength: 25,
    ...
    columns: [
      {
        ...
        orderable: false,
        render: function (row) {
          return `<input type="checkbox" class="repeatRowCheck" value="${row.id}">`;
        }
      },
      { data: 'id' },
      { data: 'client' },
...
function getSelectedRepeatIds() {
  const ids = [];
  document.querySelectorAll('.repeatRowCheck:checked').forEach(ch => ids.push(ch.value));
  return ids;
}

// ---- Ensure modals exist once ----
ensureRepeatSettingsModal();
ensureRepeatListModal();

// ---- Delegated button bindings (prevents "Next not working") ----
document.addEventListener('click', function(e){

  // Open settings
  if (e.target && e.target.id === 'repeatBtn') {
    ensureRepeatSettingsModal();
    setWeekInputToNextWeek();
    new bootstrap.Modal(document.getElementById('repeatSettingsModal')).show();
  }

  // Next -> load list modal
  if (e.target && e.target.id === 'repeatNextBtn') {
    const weekfrom = document.getElementById('weekfrom')?.value || '';
    const repeating = document.getElementById('repeatFrequency')?.value || 'Once';

    if (!weekfrom) { alert('Please select a week.'); return; }

    document.getElementById('repeatListInfo').textContent =
      `Week: ${weekfrom} | Repeat: ${repeating}`;

    initRepeatTable(repeating);

    bootstrap.Modal.getInstance(document.getElementById('repeatSettingsModal'))?.hide();
    setTimeout(() => new bootstrap.Modal(document.getElementById('repeatListModal')).show(), 150);
  }

  // Select all / none
  if (e.target && e.target.id === 'repeatSelectAll') {
    document.querySelectorAll('.repeatRowCheck').forEach(ch => ch.checked = true);
  }
  if (e.target && e.target.id === 'repeatSelectNone') {
    document.querySelectorAll('.repeatRowCheck').forEach(ch => ch.checked = false);
  }

  // Check all toggle in header
  if (e.target && e.target.id === 'repeatCheckAll') {
    document.querySelectorAll('.repeatRowCheck').forEach(ch => ch.checked = e.target.checked);
  }

  // Copy selected
  if (e.target && e.target.id === 'copyRepeatBtn') {
    const ids = getSelectedRepeatIds();
    const weekfrom = document.getElementById('weekfrom')?.value || '';
    const repeating = document.getElementById('repeatFrequency')?.value || 'Once';
    ...
    .then(resp => {
      if (!resp?.data || resp.data.status !== 'ok') throw new Error(resp?.data?.message || 'Copy failed');
      alert(`Copied: ${resp.data.inserted} record(s).`);
      repeatDT?.ajax.reload(null, false);
      // If needed: calendar.refetchEvents();
    })
    .catch(err => {
      console.error(err);
      alert('Failed to copy repeat records.');
    });
  }
    ...
    function diffHoursFromIso(startIso, endIso) {
    ...
    function openCancelHoursModal() {
      ...
      // Prefer DB values from extendedProps; fallback to selectedEvent.start/end
      const stIso = selectedEvent?.extendedProps?.stime
        || toInputLocal(selectedEvent.start)
        || '';
    
      const etIso = selectedEvent?.extendedProps?.etime
        || (selectedEvent.end ? toInputLocal(selectedEvent.end) : '')
        || '';
      ...
      // --- auto total hours from stime/etime ---
      let hours = diffHoursFromIso(stIso, etIso);
      hours = roundToQuarter(hours);
    
      // enforce minimum 0.25 if you want
      if (hours > 0 && hours < 0.25) hours = 0.25;
      ...
      hoursInput2.value = hours > 0 ? hours : '';
    
      // Reset reason
      document.getElementById('cancelReasons').value = '';
    
      // Pre-calc previews immediately if hours available
      document.getElementById('cancelClockOutPreview').textContent  =
        (hours > 0) ? addHoursToIsoLocal(stIso, hours).replace('T',' ') : '—';
    
      document.getElementById('cancelClockOutPreview2').textContent =
        (hours > 0) ? addHoursToIsoLocal(stIso, hours).replace('T',' ') : '—';
    
      // Keep your existing oninput handlers (but also mirror changes between fields if desired)
      hoursInput.oninput = function () {
        ...
        document.getElementById('cancelClockOutPreview').textContent = addHoursToIsoLocal(stIso, h).replace('T',' ');
      };
      ...
      hoursInput2.oninput = function () {
        ...
        document.getElementById('cancelClockOutPreview2').textContent = addHoursToIsoLocal(stIso, h2).replace('T',' ');
      };
