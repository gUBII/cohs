    ...
    <body class='fade-in'><?php
        ...
        if(isset($_POST["eid"])){
            ...
            if($_POST["statx"]==2){
                ...
                $sql="UPDATE project_budget_allocation 
            ...
            if($_POST["statx"]==3){
                ...
                $sql="UPDATE project_budget_allocation SET date_from='$date_from' WHERE id='".$_POST["eid"]."'";
                if ($conn->query($sql) === TRUE) $tomtom=0;
            }
            
            if($_POST["statx"]==4){
                $date_to=strtotime($_POST["date_to"]);
                $sql="UPDATE project_budget_allocation SET date_to='$date_to' WHERE id='".$_POST["eid"]."'";
                if ($conn->query($sql) === TRUE) $tomtom=0;
            }
            ...
            if($_POST["statx"]==7){
                ...
                $sql="update project_budget_allocation set item_number='".$_POST["linenumberx"]."',rate1='$supportprice' where id='".$_POST["eid"]."'";
                if ($conn->query($sql) === TRUE) $tomtom=0;
            }
            
            if($_POST["statx"]==8){
                $sql="update project_budget_allocation set category='".$_POST["category"]."',repeating='".$_POST["repeating"]."' where id='".$_POST["eid"]."'";
                if ($conn->query($sql) === TRUE) $tomtom=0;
            }
            
            if($_POST["statx"]==18){
                $rate3=($_POST["qty3"]*156.3);
                $sql="update project_budget_allocation set qty3='".$_POST["qty3"]."',rate3='$rate3' where id='".$_POST["eid"]."'";
                if ($conn->query($sql) === TRUE) $tomtom=0;
            }
        }
        ...
        if($zid!=0){
            $ln1 = "select * from project_budget_allocation where id='$zid' order by id asc limit 1";
            $ln2 = $conn->query($ln1);
            if ($ln2->num_rows > 0) { while($ln3 = $ln2 -> fetch_assoc()) {
                ...
                if($ln3["repeating"]=="Fortnight Repeat") $wd=($wd/2);
                else if($ln3["repeating"]=="Month Repeat") $wd=($wd/$months);
                        ...
                        <td style='width:10%;text-align:right;font-size:10pt' nowrap>".$ln3["qty1"].":".$ln3["qty2"]." Hours<br>";
                            ...
                            if($hd1>=1){
                                if($ln3["qty3"]==0){
                                    $qty3=$hd1;
                                    $rate3=($qty3*156.3);
                                    $sql="update project_budget_allocation set qty3='$qty3',rate3='$rate3' where id='".$ln3["id"]."'";
                                    if ($conn->query($sql) === TRUE) $tomtom=0;
                                }else{
                                    $qty3=$ln3["qty3"];
                                }
                                ...
                                echo"<form method='post' name='form_3000' action='workspace_budget_valuation.php' target='_self' enctype='multipart/form-data'class='tooltip-end-bottom'>
