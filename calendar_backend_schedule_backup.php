...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $events[] = [
            ...
            'clockin_request'   => isset($row['clockin_request']) ? $row['clockin_request'] : null,
            'clockout_request'  => isset($row['clockout_request']) ? $row['clockout_request'] : null,
        ];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'request_time') {
        ...
        $conn->query("UPDATE shifting_allocation SET clockin_request='$ci', clockout_request='$co' WHERE id='$id' LIMIT 1");
        echo json_encode(['status'=>'ok']);
        exit;
    }
    ...
    if ($action === 'cancel') {
        ...
        $upd = "
            ...
            SET
                cancelled='1',
                total_hour='$total_hour_sql',
                clockin='$clockin_sql',
                clockout='$clockout_sql'
            WHERE id='$id'
            LIMIT 1
        ";
        if (!$conn->query($upd)) {
    ...
    if ($action === 'copy') {
        ...
        $message = "$projectname, Date: $new_stime - $new_etime, Repeating: $new_repeating, Admin Note: $new_admin_note - <a href='#'>Accept Shift</a>.";
