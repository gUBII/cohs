...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["csv_file"])) {
    ...
    while (($row = fgetcsv($h)) !== false) {
        ...
        $uid        = clean($row[1]);
        $dob        = clean($row[2]);
        $referrer   = clean($row[3]);
        $hcp        = clean($row[4]);
        $caseMgr    = clean($row[5]);
        $scheduler  = clean($row[6]);
        $acctRef    = clean($row[7]);
        $ndis       = clean($row[9]);
        $birthCountry = clean($row[11]);
        $language     = clean($row[12]);
        $package      = (int) clean($row[13]); // int
        $address      = clean($row[15]);
        $phone        = clean($row[16]);
        $email        = clean($row[17]);
        $billing      = clean($row[18]);
        $note         = clean($row[21]);
        $noteStaff    = clean($row[22]);

        /* NAME SPLIT LOGIC */
        $parts = preg_split('/\s+/', $clientName);
        $title    = $parts[0] ?? "";
        $username = $parts[1] ?? $clientName;
        $username2 = count($parts) > 2 ? implode(" ", array_slice($parts, 2)) : "";
