<?php
    ...
    if(isset($_POST["uid"])){
        if(isset($_POST["utype"])){
            ...
            echo"<form method='POST' action='index.php' name='main' target='_top'>
                <input type=hidden name='url' value='admin-dashboard.php'><input type=hidden name='id' value='5138'>
            </form>";
    ...
    if(isset($_POST["uemail"])){
        if(isset($_POST["logtime"])){
            if(isset($_POST["logip"])){
                ...
                // Create database
                $sql = "CREATE DATABASE $uemail";
                if ($conn->query($sql) === TRUE) {
                    
                    // ADDING DATA TO MAIN DATABASE.
                    $sql = "insert into uerp_user (date,unbox,email,passbox,jointime,ref_db) 
                    VALUES ('$sysdate',\"$uemailx\",\"$uemailx\",'$uemail','$ip','$uemail')";
                    if ($conn->query($sql) === TRUE) $tomtom=0;
                    
                    // SETTING NEW DATABASE CONNECTION
                    $connx = new mysqli('localhost', 'root', 'root', "$uemail");
                    $query = '';
                    $sqlScript = file('saas.sql');
                    ...
                    // SETTING NEW DATABASE CONNECTION & ADDING DATE TO DATABASE
                    ...
                    $sql = "insert into uerp_user (id,date,unbox,email,passbox,jointime,ref_db) 
                    VALUES ('1','$sysdate',\"$uemailx\",\"$uemailx\",'$uemail','$ip','$uemail')";
                    ...
                    echo"<body style='padding:0px'>                
                        <div class='container-fluid p-0 h-60 position-relative bg-foreground'><center>
                            <p id='blink_text' style='font-size:15pt'>
                                Sorry!<br><span style='font-size:10pt'>Same Database already exists on our system...</span>
                            </p><br><br>
                        </div>
                    </body>";
                }
    ...
    if(isset($_COOKIE["useridx"])){
        ...
        $connx = new mysqli('localhost', 'root', 'root', $_COOKIE['dbnamex']);
        if ($connx->connect_error) die("Connection failed: " . $conn->connect_error);

        // echo"".$_COOKIE['dbnamex']."";
        ...
        if(isset($_POST["pbox"])){
            ...
            $sql="update uerp_user set passbox='".$_POST["pbox"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        }
        ...
        if(isset($_POST["company_name"])){
            ...
            $sql="update uerp_user set store_name='".$_POST["pcompany_namehone"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
            $sql="update uerp_user set store_name='".$_POST["pcompany_namehone"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
            $sql="update companysetting set companyname='".$_POST["company_name"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        }
        ...
        if(isset($_POST["company_address"])){
            ...
            $sql="update uerp_user set address='".$_POST["address"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
            $sql="update uerp_user set address='".$_POST["address"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["company_city"])){
            ...
            $sql="update uerp_user set city='".$_POST["city"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
            $sql="update uerp_user set city='".$_POST["city"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["company_state"])){
            ...
            $sql="update uerp_user set area='".$_POST["area"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
            $sql="update uerp_user set area='".$_POST["area"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["company_zip"])){
            ...
            $sql="update uerp_user set zip='".$_POST["zip"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
            $sql="update uerp_user set zip='".$_POST["zip"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["country"])){
            ...
            setcookie("country", $_POST["country"], time()+9999999);
            $sql="update uerp_user set country='".$_POST["country"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
            $sql="update uerp_user set country='".$_POST["country"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
            $sql="update companysetting set country='".$_POST["country"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        }
        ...
        if(isset($_POST["job_title"])){
            ...
            $sql="update uerp_user set designation='".$_POST["job_title"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
            $sql="update uerp_user set designation='".$_POST["job_title"]."' where id='1'";
            if ($connx->query($sql) === TRUE) $tomtom=0;
        }
        ...
        if(isset($_POST["total_employees"])){
            ...
            $sql="update uerp_user set total_employee='".$_POST["total_employees"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["purpose"])){
            if(isset($_COOKIE["purpose_of_use"])){
                ...
                setcookie("purpose_of_use", "", time() -99999);
            }
            setcookie("purpose_of_use", $_POST["purpose"], time()+9999999);
            $sql="update uerp_user set purpose_of_use='".$_POST["purpose"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["referred_source"])){
            ...
            $sql="update uerp_user set referred_source='".$_POST["referred_source"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["application_name"])){
            ...
            $sql="update uerp_user set applicaiton_id='".$_POST["application_id"]."' where id='1'";
            if ($conn->query($sql) === TRUE) $tomtom=0;
        ...
        if(isset($_POST["module_selection"])){
            ...
            $sql="update modules set dashboard='0' where id>='1'";
            if ($connx->query($sql) === TRUE) $tomx=0;

            $sql="update modules set dashboard='1' where parent='".$_POST["application_name"]."'";
            if ($connx->query($sql) === TRUE) $tomx=0;
        ...
        if(isset($_POST["uid2"])){
            if(isset($_POST["utype2"])){
                ...
                echo"<form method='POST' action='index.php' name='main' target='_top'>
                    <input type=hidden name='url' value='global-dashboard.php'><input type=hidden name='id' value='5138'>
                </form>";
                ?> <script language=JavaScript> document.main.submit(); </script> <?php
            }
        }
