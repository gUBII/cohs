<?php
    ...
    if($_POST["processor"]=="edittask"){
        $datex=strtotime($_POST["date1"]);
        $datey=strtotime($_POST["date2"]);
        $title = str_replace("'", "`", $_POST["title"]);
        $detail = str_replace("'", "`", $_POST["detail"]);
        $randid=time();
        $sql="update tasks set employeeid='".$_POST["employeeid"]."',clientid='".$_POST["clientid"]."',title='$title',
        ...
        if ($conn->query($sql) === TRUE){
            ...
            $recid=0;
            $sql = "SELECT * FROM tasks where id='".$_POST["id"]."' order by id desc limit 1";
            ...
            VALUES ('".$_COOKIE["userid"]."','0','0','TASK UPDATED','".$_POST["priority"]."','$sysdate','$ip','1','Task Updated on $title','".$_POST["employeeid"]."','".$_POST["clientid"]."')";
    ...
    if($_POST["processor"]=="tasklist"){
        ...
        $detail = str_replace("'", "`", $_POST["detail"]);
        $randid=time();
        $sql = "insert into tasks (employeeid,clientid,date,title,detail,priority,image,tdate,mode,start,end,projectid) 
