...
if ($action === 'template_apply_at') {
    ...
    if ($tplId <= 0 || $dropStartIso === '') {
        http_response_code(400);
        echo json_encode(['status'=>'error','message'=>'Missing template_id or drop_start']);
        exit;
    }
    $tplQ = $conn->query("SELECT * FROM shifting_allocation_table WHERE id='$tplId' LIMIT 1");
    if (!$tplQ || $tplQ->num_rows === 0) { http_response_code(404); echo json_encode(['status'=>'error','message'=>'Template not found']); exit; }
    ...
    if (!$dropTs) { http_response_code(400); echo json_encode(['status'=>'error','message'=>'Invalid drop_start']); exit; }

    $dropDate = date('Y-m-d', $dropTs);
    $tplTime = '00:00:00';
    if (!empty($tpl['stime'])) { $tplTime = date('H:i:s', strtotime($tpl['stime'])); }
    ...
    if ($tplStartTs > 0 && $tplEndTs > 0 && $tplEndTs > $tplStartTs) $dur = $tplEndTs - $tplStartTs;
    else {
        $a = strtotime($tpl['stime']); $b = strtotime($tpl['etime']);
        if ($a && $b && $b > $a) $dur = $b - $a;
    }
    if ($dur <= 0) $dur = 3600;
    ...
    $sDays = date('d', $newStartTs); $sMonths = date('m', $newStartTs); $sYears = date('Y', $newStartTs);
    $eDays = date('d', $newEndTs);   $eMonths = date('m', $newEndTs);   $eYears = date('Y', $newEndTs);
    ...
    ";
    if (!$conn->query($q)) { http_response_code(500); echo json_encode(['status'=>'error','message'=>'DB insert failed']); exit; }
